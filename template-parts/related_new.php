...
// If no related posts and the override is set to 'yes', adjust the query to fetch posts from the same category.
if (!$relatedPosts->have_posts() && $related_post_override == 'yes') {
    $categories = wp_get_post_categories(get_the_ID());
    $args['post__in'] = null;  // Reset post__in
    $args['category__in'] = $categories;
    $args['post__not_in'] = array(get_the_ID());  // Exclude current post
    $relatedPosts = new WP_Query($args);
}
